...
tail -n +2 "$SAMPLE_SHEET" | while IFS=, read -r data_id h5ad_path markers_path cxg_flag var_col_arg cluster_header_arg species chemistry endo_labels; do
    ...
    sbatch \
        ...
        --job-name="NSF_combined_markers_${data_id}" \
        master_script.sh \
        --data_id "$data_id" \
        --data_path "$h5ad_path" \
